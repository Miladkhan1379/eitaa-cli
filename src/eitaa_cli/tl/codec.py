...
from eitaa_cli.api_types import TLObject, TLParams, TLValue, int_field, require_object, str_field
...
class _Reader:
    ...
    def u32(self) -> int:
        return cast(int, struct.unpack("<I", self.take(4))[0])
    ...
    def f64(self) -> float:
        return cast(float, struct.unpack("<d", self.take(8))[0])
...
class TLCodec:
    ...
    def encode_method(self, method: str, params: TLParams | None = None) -> bytes:
        definition = self.schema.method(method)
        return self._encode_definition(definition, params or {})

    def encode_constructor(self, predicate: str, values: TLParams | None = None) -> bytes:
        definition = self.schema.constructor(predicate)
        return self._encode_definition(definition, values or {})

    def decode_response(self, method: str, payload: bytes) -> TLValue:
        definition = self.schema.method(method)
        if len(payload) >= 4:
            ...
            if error_definition and constructor_id == error_definition.id & 0xFFFFFFFF:
                value = require_object(self.decode_any(payload), context="RPC error")
                raise EitaaRPCError(
                    int_field(value, "code"),
                    str_field(value, "text", "RPC_ERROR"),
                    method,
                )
        reader = _Reader(payload)
        return self._decode_type(reader, definition.result_type)

    def decode_method_request(self, method: str, payload: bytes) -> TLObject:
    ...
    def decode_any(self, payload: bytes) -> TLValue:
        return self._decode_object(_Reader(payload), expected_type="Object")

    def _encode_definition(self, definition: TLDefinition, values: TLParams) -> bytes:
        ...
        for param in definition.params:
            ...
            if param.type == "#":
                out.extend(
                    struct.pack(
                        "<I",
                        flags.get(param.name, _coerce_int(values.get(param.name, 0), param.name)),
                    )
                )
                continue
            if optional:
    ...
    @staticmethod
    def _calculate_flags(params: Sequence[TLParam], values: TLParams) -> dict[str, int]:
        flags: dict[str, int] = {
            p.name: _coerce_int(values.get(p.name, 0), p.name) for p in params if p.type == "#"
        }
        for param in params:
            optional = _optional(param.type)
            if not optional:
    ...
    def _encode_type(self, type_name: str, value: object, *, field: str) -> bytes:
        ...
        if vector is not None:
            ...
            return bytes(out)
        if type_name in {"int", "#"}:
            return struct.pack("<i", _coerce_int(value, field))
        if type_name == "long":
            return struct.pack("<q", _coerce_int(value, field))
        if type_name == "double":
            return struct.pack("<d", _coerce_float(value, field))
        if type_name == "int128":
    ...
    def _encode_object(self, value: object, field: str, expected_type: str | None = None) -> bytes:
    ...
    def _decode_type(self, reader: _Reader, type_name: str) -> TLValue:
        type_name = _strip_bare(type_name)
        vector = _vector_item(type_name)
        if vector is not None:
    ...
    def _decode_object(self, reader: _Reader, expected_type: str) -> TLValue:
        constructor_id = reader.u32()
    ...
    def _decode_params(self, reader: _Reader, definition: TLDefinition) -> TLObject:
        result: TLObject = {}
        flags: dict[str, int] = {}
        for param in definition.params:
            optional = _optional(param.type)
...
def _coerce_int(value: object, field: str) -> int:
    ...
    if isinstance(value, str):
        try:
            ...
            raise TLCodecError(f"{field} must be an integer") from exc
    raise TLCodecError(f"{field} must be an integer")
...
def _coerce_float(value: object, field: str) -> float:
    ...
    if isinstance(value, str):
        try:
            ...
            raise TLCodecError(f"{field} must be a number") from exc
    raise TLCodecError(f"{field} must be a number")
